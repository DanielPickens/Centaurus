...
func createEnvDirAndFile() {
	ensureDirExists(homedir.HomeDir(), appConfigDir)
	ensureDirExists(homedir.HomeDir(), appConfigDir, appKubeConfigDir)
}

func ensureDirExists(pathSegments ...string) {
	dirPath := filepath.Join(pathSegments...)
